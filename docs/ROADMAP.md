...
- Task 005: Notion API 연동 및 데이터 조회 구현 ✅ - 완료
  - ✅ `@notionhq/client` 의존성 설치 및 `get-invoice.ts` 파일 골격 작성
  - ✅ 환경변수 미설정 시 mockInvoice로 폴백하는 개발 편의 로직 구현
  - ✅ Notion 데이터베이스를 **Invoice/Items 2-DB 관계형 구조**로 실제 생성 완료 (Notion API로 자동 생성, Items를 Relation 속성으로 연결)
  - ✅ Notion 페이지 → Invoice/InvoiceItem 타입 매핑 함수 구현 (`src/lib/notion/map-invoice.ts`, `mapPageToInvoice`/`mapPageToInvoiceItem`/`getRelatedItemPageIds`) — `isFullPage` 가드, 프로퍼티 타입별 안전 추출 헬퍼 포함
  - ✅ `getInvoiceByToken`에서 Items Relation을 따라 품목 페이지를 병렬 조회 후 매핑, try/catch 에러 처리 추가 (`src/lib/notion/get-invoice.ts`)
  - ✅ `NOTION_ITEMS_DATABASE_ID` 환경변수 추가 (`src/lib/env.ts`, `.env.example`, `.env.local`)
  - ✅ 실제 Notion 워크스페이스에 테스트 견적서(`real-test-token-001`) + 품목 2건을 생성해 실 데이터 기반 전체 플로우 검증 완료
  - **## 테스트 체크리스트 (Playwright MCP)**
    - [x] 정상 케이스: 실제 Notion 데이터 기반 `/invoice/real-test-token-001` 접속 → 견적서 정보/품목 2건/총액/비고 정상 렌더링 확인
    - [x] 실패 케이스: 존재하지 않는 publicToken으로 접속 시 404 페이지 노출 확인 (mock 및 실 Notion 연동 상태 양쪽에서 확인)
    - [x] 실패 케이스: `NOTION_API_KEY` 미설정 상황에서 mockInvoice 폴백 정상 동작(회귀 없음) 확인
    - [x] 엣지 케이스: 품목이 0개이거나 일부 프로퍼티(비고, 유효기간, 통화 등)가 비어있는 경우 크래시 없이 안전한 기본값으로 폴백하는지 단위 검증 완료 (`mapPageToInvoice` 격리 테스트)
    - [x] 엣지 케이스: 특수문자(`&`, `<b>` 등)가 포함된 발주처명·품명이 실제 Notion 데이터에서도 정상 표시되는지 확인
- Task 006: PDF 다운로드 기능 실데이터 검증
  ...
  - ✅ Notion 실데이터(`real-test-token-001`) 기준 PDF 다운로드 재검증 완료 (`GET /api/invoice/real-test-token-001/pdf 200`, Playwright 다운로드 트리거 확인)
  - PDF 내 한글 폰트 깨짐 여부 육안 확인 및 필요 시 커스텀 폰트 등록 (다운로드된 PDF 파일을 직접 열어서 확인 필요)
  - **## 테스트 체크리스트 (Playwright MCP)**
    - [x] 정상 케이스: PDF 다운로드 버튼 클릭 → 파일 다운로드 완료 확인 (실 Notion 데이터 기준)
    - [ ] 정상 케이스: 다운로드된 PDF 내 한글(발주처명, 품명, 비고 등) 깨짐 없이 표시되는지 육안 확인
    - [x] 실패 케이스: 존재하지 않는 토큰의 PDF 라우트 직접 호출 시 적절한 에러 응답(404 등) 확인
    - [ ] 엣지 케이스: 품목 수가 많아 페이지가 넘어가는 견적서의 PDF 페이지네이션 정상 동작 확인
    - 남은 체크리스트(한글 폰트 육안 확인, 페이지네이션) 통과 후에만 Task 완료로 최종 표시
