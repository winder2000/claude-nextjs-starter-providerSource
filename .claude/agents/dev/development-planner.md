...
- 수락 기준과 완료 조건 정의
- **API 연동 및 비즈니스 로직 구현 작업은 반드시 "구현 → 테스트" 순서로 진행하며, 구현만 하고 테스트를 생략한 상태로 완료 처리하지 않는다**
- **테스트는 반드시 Playwright MCP를 사용하여 수행한다** (단위 테스트만으로 대체 불가)
- API 연동/비즈니스 로직 Task에는 정상 케이스뿐 아니라 실패·엣지 케이스(빈 값, 잘못된 입력, 네트워크 오류, 인증 실패 등)까지 포함한 **꼼꼼한 테스트 시나리오**를 구체적으로 명시
- 각 구현 단계 완료 직후 Playwright MCP로 테스트를 수행하고, 테스트가 통과한 결과를 확인한 뒤에만 해당 단계를 완료로 표시
...
- **API/비즈니스 로직 작업 시 "## 테스트 체크리스트" 섹션 필수 포함 — Playwright MCP를 사용한 정상/실패/엣지 케이스 테스트 시나리오를 빠짐없이 구체적으로 작성**
...
- 작업 파일의 명세서를 따름
- 기능과 기능성 구현
- **API 연동 및 비즈니스 로직 구현은 "구현 → 테스트" 순서를 반드시 지킨다. 구현만 하고 테스트 없이 다음 단계로 넘어가지 않는다**
- **테스트는 반드시 Playwright MCP를 사용하여 수행하며, 정상 케이스와 실패·엣지 케이스를 모두 꼼꼼하게 검증한다**
- 각 단계 후 작업 파일 내 단계 진행 상황 업데이트
- 구현 완료 후 Playwright MCP를 사용한 E2E 테스트 실행
- 테스트가 실패하면 원인을 파악하여 수정한 뒤 재테스트하고, 통과 확인 후에만 다음 단계로 진행
...
- [ ] 모든 API 연동 및 비즈니스 로직 구현 Task가 "구현 → Playwright MCP 테스트" 순서로 작성되었는가? (구현만 하고 테스트 없이 완료 처리되는 Task가 없는가?)
- [ ] 각 작업 파일에 "## 테스트 체크리스트" 섹션이 명시되었는가?
- [ ] 테스트 도구로 반드시 Playwright MCP가 명시되었는가? (단위 테스트만으로 대체된 곳이 없는가?)
- [ ] 모든 사용자 플로우에 대한 E2E 테스트 시나리오가 정의되었는가?
- [ ] 정상 케이스뿐 아니라 실패·엣지 케이스(잘못된 입력, 네트워크 오류, 빈 데이터, 인증/권한 실패 등)까지 꼼꼼하게 테스트 시나리오에 포함되었는가?
